...
const AddUserPopup = () => {
  ...
  const [uplineUserType] = useState("MasterAgent");
  ...
  const handleUserCommissionChange = (e) => {
    const value = e.target.value;
    if (value === '' || (parseInt(value) <= maxUserCommission && !isNaN(value))) {
  ...
  return (
    <>
      ...
      {loading && <Loading />}
      <button onClick={openModal} className=" text-xs lg:text-sm h-8 px-4 rounded-sm bg-white flex items-center gap-1 text-[#1e1e1e]"><FontAwesomeIcon icon={faAdd}/><p>Add User</p></button>
      {modalVisible && (
        <div id="createModal" className="fixed inset-0 flex items-center justify-center z-50 text-xs lg:text-sm">
          <div className="bg-[#eee] rounded-lg shadow-lg px-6 py-3 w-96 relative">
            ...
            <h3 id="createHeader" className="text-lg font-semibold mb-6">Add User</h3>
            <ul className="space-y-4 px-6">
              ...
              <li className='flex justify-between items-center'>
        <label className="block font-medium w-[112px] text-end">User Commission(%)</label>
        <input
          id="userCommission"
          type="number"
          placeholder="max 2"
          value={userCommission}
          onChange={handleUserCommissionChange}
          className="form-input shadow inset-x-0 inset-y-1 text-opacity-50 px-2 py-[2px] ml-4"
        />
        {userCommission > maxUserCommission && <span className="error-text">Value cannot be greater than {maxUserCommission}.</span>}
      </li>
      <li className='flex justify-between items-center'>
        <label className="block font-medium w-[112px] text-end">Agent Commission(%)</label>
        <input
          id="agentCommission"
          type="number"
          placeholder="max 3"
          ...
          className="form-input shadow inset-x-0 inset-y-1 text-opacity-50 px-2 py-[2px] ml-4"
        />
        {agentCommission > maxAgentCommission && <span className="error-text">Value cannot be greater than {maxAgentCommission}.</span>}
      </li>
              <li className=' flex justify-between items-center'>
                <label className="block font-medium w-[112px] text-end">User Reference</label>
                  ...
                  className="form-input shadow inset-x-0 inset-y-1 text-opacity-50 px-2 py-[2px] ml-4"
                    ...
                    placeholder="Enter"
                    maxLength="16"
                    onChange={(e) => setUserReferrence(e.target.value)}
                    readOnly="readonly"
                    onFocus={(e) => e.target.removeAttribute('readonly')}
                  />
